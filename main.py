...
class MyApp(QMainWindow, Ui_Dialog):
    ...
    def eventFilter(self, source, event):
        ...
        if source == self.sel_ali:
            if event.type() == QEvent.DragEnter:
                ...
                self.sel_ali.setText(file_path)
                self.sel_ali_path.setText("未选择文件夹")
                return True

        if source == self.sel_ali_2:
            if event.type() == QEvent.DragEnter:
                if event.mimeData().hasUrls():
                    event.acceptProposedAction()
                return True
                ...
                for url in urls:
                    files.append(url.toLocalFile())
                    file_path = ', '.join(files)
                self.sel_ali_2.setText(file_path)
                self.sel_ali_path_2.setText("未选择文件夹")
                return True

        if source == self.cds_input_text:
            if event.type() == QEvent.DragEnter:
                if event.mimeData().hasUrls():
                    event.acceptProposedAction()
                return True
    ...
    def on_button_open_file_clicked(self, label):
        file_name, _ = QFileDialog.getOpenFileName(None, "请选择你的序列文件:)", "", "All Files (*)")
        if file_name:
            # 在这里处理选定的文件，例如打印文件名
