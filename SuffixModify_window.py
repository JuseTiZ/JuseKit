...
from UIs.SuffixmodifyQtUI import wzkit
from utils.windowAction import show_message_dialog
...
class Wzapp(QDialog, wzkit):
    ...
    def browse_folder(self):
        ...
        if folder_path:
            ...
            if any(any(file.endswith(end) for end in end_if) for file in files):
                show_message_dialog(text = "检测到有特定尾缀的文件，已为你自动填写。")
    ...
    def batch_rename_files(self):
        ...
        if folder_path == '未选择文件夹':
            show_message_dialog(text="你一定要什么时候都尝试一下空Run吗？")
            return
        if old_suffix == '':
            show_message_dialog(text="原有尾缀不能填空哦不然会出问题")
            return
        ...
        show_message_dialog(text="批量替换成功")
